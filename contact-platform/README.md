...
For production on `contact.nexora.kg`, set strong `POSTGRES_PASSWORD` and `SESSION_SECRET` in `.env`; root password still comes from `/etc/nexora-contact.env` `ADMIN_PASSWORD`.

Login as:

- user: `root`
- password: value from `CONTACT_ROOT_PASSWORD` or `/etc/nexora-contact.env` `ADMIN_PASSWORD`
